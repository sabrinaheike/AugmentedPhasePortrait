...
function [] = augmented_implicit(fin,gin, minx, maxx, miny, maxy, options)
    ...
    for i = 1:nr_nullclinesX
        ...
        xreal_rounded = round(xreal,decimal);

        yvals = XbranchesYdata(:,i);
        yreal = yvals(~isnan(yvals));
        yreal = round(yreal,10);
        yreal_rounded = round(yreal,decimal);

        % Check if branch is a function of x or y 
        xdir = diff(xreal);
        ydir = diff(yreal);

        xdir_rounded = diff(xreal_rounded);
        ydir_rounded = diff(yreal_rounded);

        xfunc = 0;
        yfunc = 0;
        if all(xdir_rounded < 0) || all(xdir_rounded>0)
            xfunc = 1;
        elseif all(ydir_rounded < 0) || all(ydir_rounded>0) % y values change in constant direction
            yfunc = 1;
        elseif all(xdir < 0) || all(xdir>0) % x values change in constant direction
...
function [combinationOptionsX, combinationOptionsY, nrNullclinesOptions] = AttemptNullclineCombine(branchDataX, branchDataY, nrNullclines, XMax, XMin, YMax, YMin, optimize)
        ...
        % specific
        closenessValue = 22;
        distmax = sqrt(((XMax-XMin)/(closenessValue))^2 + ((YMax-YMin)/(closenessValue))^2);
        ...
        for i = 1:2*nrNullclines
            for j = 1:2*nrNullclines
                if i >= j || j == i + nrNullclines
                    distances(i,j) = distmax + 1;
                end
            end
        end

        %find distances smaller than min distance 
        [iIdx, jIdx] = find(distances <= distmax);
