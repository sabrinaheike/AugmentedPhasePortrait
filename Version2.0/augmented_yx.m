...
function[ ] = augmented_yx(fin,gin, minx, maxx, miny, maxy, varargin)
    ...
    for i = 1:length(x_toplot)
        ...
        if isnumeric(eval(h))==1                   
            % constant nullcline
            hold on
            plot([eval(h) eval(h)],[miny,maxy], 'LineStyle', '--', 'Color', XColours(i,:), 'LineWidth', 3,'DisplayName','X-Nullcline');
        else
            hf = matlabFunction(h);
            hold on
    ...
    for j = 1:length(y_toplot)
        ...
        if isnumeric(eval(k))==1
            ...
            kf = matlabFunction(k);
            hold on
            fplot(kf, [minx,maxx],'LineStyle', '--', 'Color', YColours(j,:), 'LineWidth', 2.5,'DisplayName','Y-Nullcline');
        end
    end
