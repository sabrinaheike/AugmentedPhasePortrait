...
function[ ] = augmented_yy(fin,gin, minx, maxx, miny, maxy, varargin)
        ...
        if ~isempty(XnullclineInfo{i,2}) %if asymptote in nullcline
            for n=i+1:nr+2:length(xaxis)
                for m = i+2:nr+2:length(yaxis)
                    xt = xaxis(n);
                    yt = yaxis(m);
                    if gf(xt,yt) > XnullclineInfo{i,2}(end) %point above all asymptotes
        ...
        if ~isempty(YnullclineInfo{j,2}) %if asymptote in nullcline
            for n=j+2:nr+2:length(xaxis)
                for m = j+1:nr+2:length(yaxis)
                    xt = xaxis(n);
                    yt = yaxis(m);
                    if gf(xt,yt) > YnullclineInfo{j,2}(end)
    ...
    for i = 1:size(XnullclineInfo,1)
        ...
        if isnumeric(eval(h))==1
            % constant nullcline
            hold on
            plot([eval(h) eval(h)],[miny,maxy],'LineStyle', '--', 'Color', XnullclineInfo{i,4}, 'LineWidth',3,'DisplayName','Y-Nullcline');
        else
            hf = matlabFunction(h);
    ...
    for j = 1:size(YnullclineInfo,1)
        ...
        if isnumeric(eval(k))==1
            ...
            for m = 1:length(yIntervals) - 1
                if yIntervals(m+1) - yIntervals(m) > 2*plotTolerance
                    hold on
                    fimplicit(kf-x,[minx maxx eval(yIntervals(m)+plotTolerance) eval(yIntervals(m+1)-plotTolerance)],'LineStyle', '--', 'Color', YnullclineInfo{j,4}(m,:), 'LineWidth', 2.5,'MeshDensity', 500,'DisplayName','Y-Nullcline');
                end
            end
        end
    end
