...
function[ ] = augmented_xy(fin,gin, minx, maxx, miny, maxy, varargin)
    ...
    for j = 1:size(YnullclineInfo,1)
        ...
        asymptoteList = wa(wa>miny);
        asymptoteList = asymptoteList(asymptoteList<maxy);
        [warnMsg, ~] = lastwarn;
        if ~isempty(warnMsg) %if there was a warning using solve
            tolerance = 0.001;
            % try alternate method to solve for asymptotes
            asymptote(Dkd,miny+tolerance,maxy-tolerance,y)
        end
        if ~isempty(asymptoteList)
            disp('take care there is a vertical asymptote (grey dashed line) of this nullcline but I do not distinguish colors');
            disp(k);
    ...
    for j = 1:size(YnullclineInfo,1)
        ...
        hold on
        fimplicit(Lk,'Color',mean(YnullclineInfo{j,4},1),'MeshDensity', 500,'LineWidth',1.5,'HandleVisibility','off');
    end
        ...
        if ~isempty(YnullclineInfo{j,2}) %if asymptote in nullcline
            for n=j+2:nr+2:length(xaxis)
                for m = j+1:nr+2:length(yaxis)
                    xt = xaxis(n);
                    yt = yaxis(m);
                    if gf(xt,yt) > YnullclineInfo{j,2}(end) %point mapped above all asymptotes
                        % set symbol colour to the colour of the last
                        % nullcline branch
                        ...
                        for r = 1:length(YnullclineInfo{j,2})                        
                            if gf(xt,yt) < YnullclineInfo{j,2}(r)
    ...
    for i = 1:size(XnullclineInfo,1)
         ...
         if isnumeric(eval(h))==1
             ...
             hf = matlabFunction(h);
             xIntervals = [minx; XnullclineInfo{i,2}; maxx];
             ...
             for m = 1:length(xIntervals) - 1 %plot each branch of nullcline in its assigned colour
                if xIntervals(m+1) - xIntervals(m) > 2*plotTolerance
                    hold on
                    fplot(hf, [eval(xIntervals(m)+plotTolerance),eval(xIntervals(m+1)-plotTolerance)],'LineStyle', '--','Color', XnullclineInfo{i,4}(m,:),'LineWidth', 3,'DisplayName','X-Nullcline'); %;,'LineStyle', '--','Color', CMH(i+counth,:),'LineWidth', 2,'DisplayName','X-Nullcline'); 
                end
             end
         end
    end
    ...
    for j = 1:size(YnullclineInfo,1)
        ...
        if isnumeric(eval(k))==1
            %constant nullcline
            hold on
            plot([eval(k) eval(k)],[miny, maxy], 'LineStyle', '--', 'Color', YnullclineInfo{j,4}, 'LineWidth', 2.5,'DisplayName','Y-Nullcline');
        else
            kf = matlabFunction(k);
            yIntervals = [miny; YnullclineInfo{j,2}; maxy];
            plotTolerance = (maxy-miny)/10000;
            for m = 1:length(yIntervals) - 1
                if yIntervals(m+1) - yIntervals(m) > 2*plotTolerance
                    hold on
                    fimplicit(kf-x,[minx maxx eval(yIntervals(m)+plotTolerance) eval(yIntervals(m+1)-plotTolerance)],'LineStyle', '--', 'Color', YnullclineInfo{j,4}(m,:),'MeshDensity', 500, 'LineWidth', 2.5,'DisplayName','Y-Nullcline');
                end
            end
        end
    end
    ...
    for j=1:size(YnullclineInfo,1)
        for r = 1:length(YnullclineInfo{j,2})
            plot([minx maxx],[eval(YnullclineInfo{j,2}(r)), eval(YnullclineInfo{j,2}(r))], 'LineStyle', '--', 'Color', [0.5, 0.5, 0.5], 'LineWidth', 3, 'HandleVisibility','off');
        end
    end
    ...
    % intersection between 2 trivial nullclines
        % plot equilibrium point at (0,0) if x-axis is nullcline for
        % y-equation and y-axis is nullcline for x-equation
    if trivh>0 && countk>0
        % make sure that neither of the axes are all equilibria points
        if trivk==0 && counth==0
            if subs(Df,y,0) ~= 0 && subs(Dg,x,0) ~= 0
